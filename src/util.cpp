...
const char * const BITCOIN_CONF_FILENAME = "kotyacoin.conf";
const char * const BITCOIN_PID_FILENAME = "kotyacoin.pid";
...
#else
    const char* pszModule = "kotyacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kotyacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Kotyacoin";
#else
    // Unix
    return pathRet / ".kotyacoin";
#endif
#endif
}
